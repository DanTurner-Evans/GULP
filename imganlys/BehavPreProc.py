...
def loadBehavDat(fname):
    ...
    if (fname.split('.')[1] == 'txt'): #Parsing for original behavioral data
    ...
    elif (fname.split('.')[1] == 'json'): # parsing for unity vr behavioral data
        with open(fname, 'r') as fh:
            data = json.load(fh)
            for i in range(0,len(data)):
                try:
                    data[i]['attemptedTranslation'] # check if at index with offset data
                    et.append(float(data[i]['timeSecs']))
                    ro.append(float(data[i]['rotationDegs']['y']))
                    fo.append(float(data[i]['worldPosition']['z'])) # Im not sure which is forward and which is lateral (x or z)
                    lo.append(float(data[i]['worldPosition']['x']))
                    dx0.append(0)
                    dx1.append(0)
                    dy0.append(0)
                    dy1.append(0)
...
def getMatchedBehavDat(tFramegrab, tVR, behavDatNm):
    ...
    if(behavDatNm.split('.')[1].lower() == "txt"): # Default behavioral data
        behavDat_matched = behavDat.loc[datPts].reset_index(drop=True)

    elif(behavDatNm.split('.')[1].lower() == "json"): # Unity Vr behavioral data
        behavDat_matched = behavDat.head(n=len(datPts)) # Assumes behave data is larger than sync
